...
*(This layer exists only to aid immediate recognition by organic intelligence. It introduces no rules and may be discarded without consequence.)*
...
*Note: This requirement interacts with structure maintenance, capacity limits, and externalization.*
(Flagged: Cross-constraint references to Theta, Alpha, Delta, Lambda removed for normalization).
...
*These examples do not define Ω. They illustrate it. Removal of this layer must not alter compliance determination.*
...
II — CONSTRAINT PROTOCOL (Binding · Timeless)
  ...
  **Internal Representation**  
  Any internally maintained state that participates in selecting among multiple possible actions or trajectories **across the control-relevant horizon**, however short or extended that horizon may be.
  ...
  **Omega would be falsified if there existed a system that:**
  ...
  3. **And does so while expending zero physical work to maintain or update that internal state across the control-relevant horizon**, with no equivalent cost displaced to other physical or representational domains.
  *(Flagged: Specific cross-constraint displacement references moved to Section I)*